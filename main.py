...
pygame.display.set_caption('LAMUMU JUMPER')
...
while running == True:
    ...
    blocks = []
    score_text = font.render('Highest MOO: ' + str(high_score), True, black, background)
    screen.blit(score_text, (465, 30))
    high_score_text = font.render('MOO: ' + str(score), True, black, background)
    ...
    if game_over:
        ...
        game_over_text = big_font.render("YOU GOT MOO'd (Spacebar restart)", True, red, background)
    ...
    if player_y < 750:
        player_x += x_change
    # else: 
    #     if not game_over:  # only stop music once
